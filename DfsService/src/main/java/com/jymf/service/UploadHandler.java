...
public class UploadHandler implements Handler {
    ...
    public void handle() {
        ...
        try {
            ...
            String fileName = json.getItemId()+"_"+json.getFileType()+"_"+json.getFileOrder();
            client.buildIndex(fileName,fdfsId,basePackage.getCopeCode());
            client.commit();
        } catch (JsonSyntaxException e){
            e.printStackTrace();
            statusCode = 3;
            errorMsg = "数据格式错误:Json格式错误";
        } catch (IOException e) {
            e.printStackTrace();
            statusCode = 4;
